...
from com.boyaa.RainbowCenter.web.session.driver import SessionDriver
from com.boyaa.RainbowCenter.web.session.session import SessionConfigurationError
...
#         for key, val in self._data_handler.items():
        keys = list(self._data_handler.keys())
        for key in keys:
            val = self._data_handler[key]
