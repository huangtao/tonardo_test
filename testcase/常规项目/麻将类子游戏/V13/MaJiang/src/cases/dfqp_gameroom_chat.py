...
class C27498and27499and27544_RoomFaceTalk(TestCase):
    '''
    C27498and27499and27544
    查看聊天界面显示
    首次打开聊天查看默认tab显示
    关闭聊天聊天
    '''
    ...
    def see_chat(self, is_next=False):
        ...
        for i in range(len(game_list)):
            ...
            try:
                ...
                self.game_page.screenshot("%s_chat2.png" % game_list[i].get_attribute("name"))
...
class C27502_RoomFace_Switchtab(TestCase):
    '''
    切换其他聊天tab,不操作关闭后再次打开查看默认显示
    '''
    owner = "Lucyliu"
    status = EnumStatus.Design
    priority = EnumPriority.High
    ...
    def see_chat(self, is_next=False):
        ...
        for i in range(len(game_list)):
            ...
            try:
                ...
                self.game_page.screenshot("%s_chat1.png" % game_list[i].get_attribute("name"))
                self.game_page.wait_element("头像frame").click()
                self.yuepai_page.wait_element("房间内聊天").click()
                self.game_page.screenshot("%s_chatreopen.png" % game_list[i].get_attribute("name"))
                self.game_page.wait_element("头像frame").click()
            except:
                try:
                    self.hall_page.wait_element("关闭对话框").click()
        ...
        for i in range(len(game_list)):
            ...
            try:
                ...
                while j < 2:
                    ...
                    time.sleep(3)
                    self.game_page.screenshot("%s_chat_%s.png" % (game_list[i].get_attribute("name"),j))
                    self.yuepai_page.wait_element("表情",20).click()
                    self.game_page.screenshot("%s_chat1_%s.png" % (game_list[i].get_attribute("name"),j))
                    elements = self.yuepai_page.get_elements("发送表情")
                    elements[0].click()
                    self.game_page.screenshot("%s_%s.png" % (game_list[i].get_attribute("name"),j))
                    self.start_step("退出房间")
                    self.yuepai_page.is_exist_yuepairoom()
                    j += 1
            except:
...
__qtaf_seq_tests__=[C27501_RoomFace_Enterroom]
