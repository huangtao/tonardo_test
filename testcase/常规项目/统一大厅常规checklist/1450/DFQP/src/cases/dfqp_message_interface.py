...
class C31026_DFQP_Message_Calls(TestCase):
    ...
    def pre_test(self):
        ...
        return1 = PHPInterface.send_message(int(self.casedata['mid']), "接口发送话费碎片私信1","接口发送话费碎片私信1",400003,1,0)
        ##print return1
...
class C31027_DFQP_Message_VIP(TestCase):
    ...
    def pre_test(self):
        ...
        self.common.closeactivity_switchserver(self.luadriver,"预发布")
        ##print self.casedata['mid']
        return1 = PHPInterface.send_message(int(self.casedata['mid']), "接口发送vip私信","接口发送vip私信",4,1,0)
        self.start_step("判断是否登陆")
        if not self.common.isloginuser(self.luadriver):
            self.common.loginuser(self.casedata['user'], self.casedata['password'])
            self.common.closeactivity_switchserver(self.luadriver, "预发布")
...
class C31029_DFQP_Message_Silver(TestCase):
    ...
    def pre_test(self):
        ...
        ##print coin
        return1 = PHPInterface.send_message(int(self.casedata['mid']), "交互道具私信","交互道具私信",0,2,0)
        ##print return1
        self.start_step("判断是否登陆")
        if not self.common.isloginuser(self.luadriver):
            self.common.loginuser(self.casedata['user'], self.casedata['password'])
    ...
    def run_test(self):
        ...
        # self.hall_page.wait_element("头像").click()
        # self.hall_page.wait_element("同步标志")
        dict1 = PHPInterface.get_user_info(self.casedata['mid'])  # 获取玩家信息
...
class C31030_DFQP_Message_Gold(TestCase):
    ...
    def pre_test(self):
        ...
        ##print crystal
        return1 = PHPInterface.send_message(int(self.casedata['mid']), "金条私信","金条私信",1,1,0)
        ##print return1
        self.start_step("判断是否登陆")
        if not self.common.isloginuser(self.luadriver):
            self.common.loginuser(self.casedata['user'], self.casedata['password'])
...
class C31031_DFQP_Message_Object(TestCase):
    ...
    def pre_test(self):
        ...
        self.common.closeactivity_switchserver(self.luadriver,"预发布")
        ##print self.casedata['mid']
        return1 = PHPInterface.send_message(int(self.casedata['mid']), "实物私信","实物私信",10045,1,0)
        ##print return1
        self.start_step("判断是否登陆")
        if not self.common.isloginuser(self.luadriver):
            self.common.loginuser(self.casedata['user'], self.casedata['password'])
...
class C31032_DFQP_Message_Three(TestCase):
    ...
    def pre_test(self):
        ...
        self.common.closeactivity_switchserver(self.luadriver,"预发布")
        ##print self.casedata['mid']
        return1 = PHPInterface.send_message(int(self.casedata['mid']), "3种以上物品私信","3种以上物品私信",[10045,0,1],1,0)
        ##print return1
        self.start_step("判断是否登陆")
        if not self.common.isloginuser(self.luadriver):
            self.common.loginuser(self.casedata['user'], self.casedata['password'])
...
# __qtaf_seq_tests__ = [C006_DFQP_Message_Object]
