...
class C31148_DFQP_GameList_DownLoad(TestCase):
      ...
      def run_test(self):
          ...
          if (self.game_page.game_is_exist("川味斗地主") == True):
          ...
          if (self.game_page.game_is_exist("血流成河") == True):
          ...
          if (self.game_page.game_is_exist("拖拉机") == True):
          ...
          if (self.game_page.game_is_exist("干瞪眼") == True):
...
class C31150_DFQP_GameNameList(TestCase):
    ...
    def run_test(self):
        ...
        #前置条件：至少已下载4个游戏
        self.start_step("点击川味斗地主：")
        if (self.common.game_page("川味斗地主") == True):
            self.game_page.wait_element("川味斗地主").click()
            time.sleep(3)
            self.game_page.game_is_download()
            time.sleep(3)
...
class C31185_DFQP_Game_Bankrupt(TestCase):
    ...
    def pre_test(self):
        ...
        global mid
        mid = self.common.get_config_value("casecfg", "mid")
        self.common.recover_user(mid=mid, value='10000')
        # 初始化Luadriver
        self.luadriver = self.common.setupdriver()
        # 关闭弹框
        self.common.closeactivity(self.luadriver)
