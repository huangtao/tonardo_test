...
class D25755_ThreeRoom_Dropfile_Display(TestCase):
    '''
    房间内降场
    '''
    owner = "LucyLiu"
    ...
    def run_test(self):
        ...
        while (int(Interface.get_money(mid)) >= 10000 or int(Interface.get_money(mid)<1000)):
            endtime = time.time()
            if (endtime - starttime) / 60 > self.timeout - 5:
                self.game_page.is_in_gameroom(self.luadriver)
                self.log_info("等待超时")
                return
            if int(Interface.get_money(mid)) >= 10000:
                self.common.set_money(mid, 11000)
                self.play_game()
        ...
        #     self.assert_equal("判断进入了底倍场", actual=int(chujichangdifen) < 300)
...
# __qtaf_seq_tests__ = [D25755_ThreeRoom_Dropfile_Display]
